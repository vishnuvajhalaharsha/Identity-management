...
contract('DocumentSharingContract', function (accounts) {
  ...
  it('should request document access for approval', async function () {
    const documentType = 'Passport';
  ...
  it('should request document access for rejection', async function () {
    const documentType = 'DriverLicense';
    const attributes = ['IssueDate'];

    await contractInstance.requestDocumentAccess('user@example.com', documentType, attributes, { from: accounts[1] });

    const requests = await contractInstance.getUserRequests(accounts[0]);
    assert.equal(requests.length, 2, 'Request not recorded');
  });
